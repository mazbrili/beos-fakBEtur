...
//	list->SetSelectionMessage(new BMessage(LIST_SEL));
	...
	view->AddChild(but_whoowes = new BButton(BRect(10,440,110,460), "nalezButWho", "Receivables odbiorcy", new BMessage(BUT_WHO)));
...
void dialNaleznosci::DoFind(void) {
	...
	// construct sql query
	BString sql = "SELECT id, name, onazwa, termin_zaplaty, zapl_kwota FROM invoice WHERE ";
	sql += "termin_zaplaty <= '";
...
void dialNaleznosci::DoPayFor(int item) {
	...
	if (it->Id() <= 0)
		return;
	BString sql = "UPDATE invoice SET zapl_kwota = DECROUND(0";
